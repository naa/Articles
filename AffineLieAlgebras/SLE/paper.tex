...
$G/A$-coset conformal field theory can be realized as a WZNW-model (with gauge group $G$) interacting with pure gauge field of gauge group $A\subset G$ \cite{gawdzki1988g,figueroa89equivalence}. If we fix $A$-gauge we have $G/A$ gauge symmetry. Then we add random gauge transformations to Schramm-Loewner evolution \cite{2011arXiv1112.4354N} similar to the case of WZNW-models \cite{bettelheim2005stochastic}. Denote by $\mathcal{K}$ the Killing form of a Lie algebra $\gf$ corresponding to a Lie group $G$ and by $t^{a}_{i}$ ($\tilde{t}^{b}_{i}$) the  generators of $\gf$-representation ($\af$-representation) corresponding to the primary field $\phi_{i}$.
...
\label{sec:acknowledgements}
 I am grateful to A. Bytsko, K. Izyurov, D. Chelkak and R. Santachiara for the discussions on SLE and WZNW models and to V.D. Lyakhovsky for careful reading of the present text.  This work  is supported by
