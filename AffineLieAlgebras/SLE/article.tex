...
We assume that $\mathcal{F}$ contains some set of primary fields $\phi_{\lambda_{i}}$ with conformal weights $\lambda_{i}$. Since we work with boundary CFT we need to add bulk fields in conjugate points $\bar z_{i}$. For future use in WZNW models we denote the conformal weights of these duplicate fields by $h_{\lambda_{i}^{*}}$.  Also we have boundary condition changing operators  $\phi$ at the tip of SLE trace and in the infinity.  We use the conformal map  $w(z):\mathbb{H}\setminus\gamma_{t}\to \mathbb{H}$ to rewrite this expression in upper half plane:
...
\begin{equation}
  \label{eq:24}
   (L_{-2}-\frac{\kappa}{2}L_{-1}^{2})\phi=0.
\end{equation}

In case of minimal models the set of primary fields is finite and all the states are obtained through field-state correspondence \cite{belavin1984ics}, \cite{difrancesco1997cft}
This equation means that 
We can rewrite it as the necessary condition on b.c.c. operator $\phi$:
\begin{equation*}
 \Longrightarrow \phi \left|0\right>  \text{has level 2 null state}, \phi\sim \phi_{1,2} \;\text{or}\; \phi_{2,1}
\end{equation*}
