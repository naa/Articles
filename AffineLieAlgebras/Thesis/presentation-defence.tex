...
\begin{frame}
  ...
  \item Модели Весса-Зумино-Новикова-Виттена и coset-модели
    ...
    \item описываются с использованием теории представлений аффинных алгебр Ли
    \end{itemize}
  \item Аффинные алгебры Ли
    \begin{itemize}
    \item важный класс бесконечномерных алгебр Ли
    \item развитая теория представлений
...
\begin{frame}
  ...
  \item Алгоритм вычисления коэффициентов ветвления реализован в пакете {\bf Affine.m} для {\it Mathematica}.
...
\begin{frame}
  ...
  \begin{multline*}
    \label{eq:4}
    S=S_0+k\Gamma=\\
    -\frac{k}{8\pi}\int_{S^2} d^2x\; Tr (\partial^{\mu}g^{-1}\partial_{\mu}g)- \frac{ik }{24\pi} \int_{B}\epsilon_{ijk} Tr\left(
    \tilde g^{-1}\frac{\partial \tilde g}{\partial y^i}
      \tilde g^{-1}\frac{\partial \tilde g}{\partial y^j}
      \tilde g^{-1}\frac{\partial \tilde g}{\partial y^k}\right) d^3y
  \end{multline*}
  $g(x):\mathbb{C}\cup \{\infty\}\sim S^{2}\to D(G) , \ k\in \mathbb{Z}$, $\tilde{g}$  -- продолжение $g$ на трехмерное многообразие $B$, ограниченное $S^{2}$. 
%$\pi_{3}(G)=\mathbb{Z} \Rightarrow k\in\mathbb{Z}, \; e^{-S[g]}$ определен однозначно.

  \begin{itemize}
  \item Конформная инвариантность, калибровочная симметрия
  \item   Токи 
    $J(z)= -k \partial_zg g^{-1}\sum_{a} J^{a}(z) t^{a}=\sum_{a} \sum _{n} J^{a}_{n} t^{a} z^{n-1}$ + тождества Уорда $\Rightarrow$ соотношения аффинной алгебры $\gfh$: 
