...
\begin{axiom}
  ...
  Для любого индекса $i\in B_{0}$ существуют независимые конформные веса $h_{i},\bar h_{i}\in \mathbb{R}$, такие, что
  \begin{equation}
    \label{eq:52}
  ...
  \end{equation}
где $w_{i}=w(z_{i})$, а $s_{i}=h_{i}-\bar h_{i}, d_{i}=h_{i}+\bar h_{i}$  -- конформный вес и скейлинговая размерность.
\end{axiom}
\begin{axiom}
