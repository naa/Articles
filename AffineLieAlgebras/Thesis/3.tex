...
consider the case when one of the stems $\Delta _{1}=\Delta _{\af}$ 
is a root subsystem.

Splint $\Delta \approx (\Delta _{1},\Delta _{2})$ is called ''injective'' if
$\Delta _{1}=\Delta _{\af}$, is a root subsystem
in $\Delta $ corresponding to a regular reductive subalgebra $\af%
\hookrightarrow \gf$.

In case of injective splint the second stem $\Delta _{\frak{s}}:=\Delta
_{2}=\Delta \setminus \Delta _{\af}$ can be translated into a product (%
\ref{product}) and it defines an injection fan $\Gamma _{\af%
\hookrightarrow \gf}$. Denote by $\Delta_{\mathfrak{s}0}$ the coimage of the second embedding $\phi:\Delta_{\mathfrak{s}0}\to \Delta_{\gf}$.
The following conjecture follows.

\begin{Cnj}
Each injective splint $\Delta \approx (\Delta _{\af},\Delta _{\frak{s}})
$ defines an injection fan with the carrier $\left\{ \xi \right\} _{\af%
\rightarrow \gf}$ fixed by the product
\begin{equation}
...
In case of injective splint we say that subalgebra $\af\hookrightarrow \gf$
splinters $\Delta $ (and call $\af$ the ''splinting subalgebra'' of $%
\gf$). In \cite{richter2008splints} splints are classified (see Appendix there)
...
In this Section we study properties of injective splints  $\Delta \approx (\Delta _{\af},\Delta _{\frak{s}})
$. It will
be demonstrated that in this case to find branching
coefficients for a splinting injection $\af\hookrightarrow
\gf$ means to find weight multiplicities of an irreducible
