...
Теперь вернемся к равенству (\ref{singular main-3}). Можно добавить к $\Phi _{\gf}^{\left( \mu \right) }$ пары формальных элементов, построенные выше, с противоположными знаками: $\epsilon \left( w\right)|_{w\in W_{\sfr}}$  и  $-\epsilon \left( w\right) |_{w\in W_{\sfr}}$. 
...
$\Delta_{\af}\equiv\Delta_{1}\subset\Delta$ и $Q_{\af}\subset Q$, где $Q$ -- корневая решетка. Неприводимые модули старшего веса алгебр 
$\gf$ и $\af$ мы обозначаем через $L^{\mu}$ и $L^{\nu}_{\af}$. Формула Вейля для характеров неприводимых модулей имеет вид
$\mathrm{ch}L^{\mu}=\frac{\Psi^{(\mu)}}{\prod_{\alpha\in\Delta}(1-e^{-\alpha})}$,
где  $\Psi^{(\mu)}=\sum_{w\in W}\epsilon(w) e^{w(\mu+\rho)-\rho}$ -- сингулярный элемент модуля, а$W$ -- группа Вейля алгебры $\gf$. Формальный характер неприводимого модуля допускает разложение
\begin{equation}
  \label{eq:133}
  \mathrm{ch} L^{\mu}=\sum_{\nu\in P_{\af}} b^{\mu}_{\nu} \mathrm{ch} L^{\nu}_{\af},
\end{equation}
где $P$, $P_{\af}$ -- весовые решетки $\gf$ и $\af$. Мы хотим рассмотреть аффинное расширение данной конструкции:
...
Воспользуемя специализацией \cite{kac1988modular,kac1984infinite,kac1990idl} и определением $\eta$-функции Дедекинда
$\eta(\tau)=q^{1/24}\prod_{n=1}^{\infty}(1-q^{n})$, где
$q=e^{2\pi i \tau}$, чтбы переписать предыдущее равенство в виде соотношения на тета-функции $$\Theta^{(\gfh)}_{\widehat{\lambda}=(\lambda,k,0)}(\tau,z)=\sum_{\xi\in Q_{\gf}+\frac{\lambda}{k}}e^{2\pi i k \left(\frac{1}{2} (\xi,\xi) \tau + (\xi,z)\right)}$$:
\begin{multline}
  \label{eq:135}
...
Также мы продемонстрировали, что сплинт для аффинных алгебр Ли ведет к новым соотношениям на тета-функции и функции ветвления для редукции на модули конечномерных подалгебр, что может быть полезно для вычислений. Возможно дальнейшее обобщение этого анализа на аффинные подалгебры и применение результатов изучению coset-моделей конформной теории поля.
