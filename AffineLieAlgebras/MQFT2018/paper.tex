...
\begin{lemma}
  For an integrable function $f$ and smooth analytic function $\varphi$ the sum is approximated by
  an integral with corrections  of order $\mathcal{o}(\varepsilon)$:
  \begin{equation}
    \label{eq:75}
    \sum_{i=0}^{M}\sum_{j=0}^{N}\varepsilon^{2}f\left(\varepsilon\sum_{k=-i}^{j}\varphi(k\varepsilon)\right)=
    \int_{0}^{a}\int_{0}^{b}f\left(\int_{-y}^{z}\varphi(x)\dx\right)\dz\dy+o(\varepsilon)
  \end{equation}

\end{lemma}

To derive the expressions for higher order terms of the expansion \eqref{eq:26} we need to expand
the logarithm, change the order of summation and rewrite the free energy in the same way as we did
before:
\begin{equation}
  \label{eq:22}
...
\end{equation}
Denote by $\Psi$ the following functions:
\begin{equation}
  \label{eq:54}
...
\begin{multline}
  ...
  \frac{1}{n^{3}}\left[
    ...
    =\frac{1}{ab\varphi(b-a)}\left[\sum_{n=1}^{\infty} \frac{1}{n^{3}}
      \tilde{\chi}''_{\varepsilon\varepsilon}(0,0)+2\varphi(b-a)\sum_{n=1}^{\infty}\tilde{\chi}''_{z\varepsilon}(0,0)\right]+
    \\
...
\end{equation}
where $q(z)$ was defined in the equation
...
\end{equation}
The derivatives of $\Psi$ behave as $\Psi_{n}^{(m)}(0)\sim n^{m-1}$ so the contribution from
...
\begin{equation}
  \label{eq:71}
  \left.\left(\frac{d}{d\varepsilon}\right)^{2} f^{+}\right|_{\varepsilon=0} =
  ...
  \left(\varphi\int_{0}^{a} \Psi_{n}^{+}(x) \dx -\varepsilon^{2} \Psi_{n}^{+}(a) +\varepsilon^{3}\Psi^{+'}_{n}(0)\right)\right|_{\varepsilon=0}
\end{equation}
...
\begin{equation}
  ...
  \left(\varphi\int_{0}^{b} \Psi_{n}^{-}(x) \dx -\varepsilon^{2} \Psi_{n}^{-}(a) +\varepsilon^{3}\Psi^{-'}_{n}(0)\right)\right|_{\varepsilon=0}
\end{equation}
At last, we need to consider $f^{\times}$. In this term we can just take a limit $\varepsilon\to 0$
and obtain
\begin{multline}
  \label{eq:73}
  \left.\left(\frac{d}{d\varepsilon}\right)^{2}
...
\end{multline}
This expression is easily computed explicitly, but is too cumbersome to present it here. 
%%  
