...
\begin{equation}
  ...
  \end{array}
\end{equation}
It is also possible to study regular subalgebras of this special subalgebras and get new non-maximal special embeddings, such as the special embedding at the Figure \ref{fig:a1a2a3}, with the orthogonal subalgebra obtained by the combination of \eqref{eq:42} and Table \ref{tab:diagrams}.

Exceptional Lie algebras subalgebras require lengthy analysis which is not important for the needs of computation of the branching coefficients.
...
Using the definition (\ref{fan-defined}) we get the following set of
the points $\Gamma$ with the corresponding values $s(\gamma+\gamma_0)$: \ref{fig:AffineA2A1Fan}.
Here we restricted the computation to the twelfth grade.
\begin{figure}[ph]
  \centering
