...
\label{sec:phys-appl}
(This part must be changed considerably. First it is necessary to restore our notations instead of the Di Francesco notations. Starting with $J^{a_j}_{-n_j}$ which is too archaic! Second but much more important: the example that you had presented refers to the case where the initial injection fan method remains unmodified. Are there any other cases where the modified fan method is necessary? Fourth, you write "we should select only those highest-weight modules of the subalgebra $\mathfrak{a}$ for which the relation (\ref{eq:111}) holds", where does this selection happens and where is it in the example? Fifth, again the notations, it is of no use to indicate the Dynkin indices of the fund. weights ("three level one dominant weights $[1,0,0],\;[0,1,0],\;[0,0,1]$" ) for the reader it is interesting what are they like here; we never place the level indices in the first place as in "with the highest weight $w_0=(1,0,0)$" see the section Notations. In the Figure 12 the irrep is written to be (1,0,1,0) -- very strange! Sixth,
in all the figures it is difficult to see anything and at the same time there are almost no explanations, even the boxes and dots are not commented. Some other corrections are made directly in the text.)
...
\end{equation}
\begin{comment}
The problem is also considered in the \cite{coquereaux2008conformal}.  
\end{comment}
...
Consider the embedding of the affine Lie algebra $\hat{A}_1$ into $\hat{A}_2$ constructed as the affine extension of the special embedding $su(2)\subset su(3)$ with the embedding index $x_e=4$. The level of the algebra (what is it ?) $\mathfrak{g}=\hat{A}_2$ is equal to one, so the level of the subalgebra $\tilde{k}=kx_e=4$.

There exist three level one dominant weights $[1,0,0],\;[0,1,0],\;[0,0,1]$ in the weight space of $\hat{A}_2$.
It is easy to see that the set $\Delta_{\bot}$ is empty. (Not only this set is empty but the orthogonal algebra is empty!)

Let us consider the representation with the highest weight
$w_0=(1,0,0)$ in details.

The set of the anomalous weights of this representation up to the sixth grade is depicted in the Figure \ref{fig:affine_A2_anom_point}. We also show the root subspace of the subalgebra $\hat{A}_1$ under consideration. (Where, how? What are the coordinates, it is very difficult to see anything even in the zero grade!)

\begin{figure}[h!tb]
  \includegraphics[width=160mm]{AffineA2_A1_anom.pdf}
  \caption{The anomalous weights of the $(1,0,1,0)$ representation of the algebra $\hat{A_2}$}
  \label{fig:affine_A2_anom_point}
\end{figure}

According to our algorithm \ref{sec:algorithm} we project the anomalous weights to the weight space of the subalgebra $\hat{A_1}$ and calculate the dimensions of the corresponding $\mathfrak{a}_{\bot}$-modules (there are no such subalgebra here!)$L^{\pi_{\mathfrak{a}_{\bot}}(\omega(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}$.
The result of these caculations up to the twelfth grade is presented in the Figure \ref{fig:AffineA2_A1_anom_proj}
\begin{figure}[h!tb]
  \centering
...
Using the recurrent relation for the anomalous branching coefficients we get the result presented in Figure \ref{fig:AffineA2_A1_branching}. (Isn't it obvious that all the zeros there make the picture unreadable?) 
\begin{figure}[p]
  \centering
...
We see that inside the main
Weyl chamber of $\hat{A}_1$ there are only two non-zero anomalous weights. (Show the Weyl chambers on the Figure.) These are the branching coefficients. So the finite reducibility theorem holds and we get the decomposition
\begin{equation}
  \label{eq:43}
...
The fan $\Gamma _{\frak{a}\subset \frak{g}}$ is shown at the Figure \ref{fig:A1+A1-A3_fan}.
\begin{figure}[h!tb]
  \includegraphics[width=150mm]{A1+A1-A3_fan.pdf}
  \caption{Fan for the special embedding $\widehat{su(2)\oplus su(2)}\subset \widehat{su(4)}$}
...
\begin{figure}[h!tb]
  ...
  \caption{Projected anomalous weights $\pi_{\mathfrak{a}}\left(\widehat{\Psi ^{\left( \mu \right) }}\right)$ for $L^{(\omega_1)}_{A_3}$}
  \label{fig:A1+A1-A3_anom}
\end{figure}

The set  $\widehat{\Psi^{(\mu)}}=\left\{\omega(\mu+\rho)-\rho;\; \omega\in W\right\}$ of the anomalous weights of the representation of the algebra $\hat{A}_3$ consists of 192 elements and its projection $\pi_{\mathfrak{a}}\left(\widehat{\Psi^{(\mu)}}\right)$ for $\mu=\omega_1$ is shown at the Figure \ref{fig:A1+A1-A3_anom}. We omit similar pictures for $\omega_0, \omega_2,\omega_3$ and include only the branching coefficients for these modules to save the space.
...
But we can consider the chain of embeddings $\mathfrak{a}\subset\mathfrak{g}_1\subset\mathfrak{g}_2$. We can use the embedding $\hat{A}_1\subset\hat{A}_2\subset\hat{A}_{?}$. Modular invariant partition function for this case could be obtained in two steps. First one can compute branching coefficients for $\hat{A}_2\subset\hat{A}_{?}$ and get partition function for $\hat{A}_2$ and then substitute the results of the previous section \ref{sec:spec-embedd-hata_1s} into it to get the new partition function for $\hat{A}_1$. 
