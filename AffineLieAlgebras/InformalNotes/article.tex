...
Thus here we show the simplest possible example of reduction of affine $\widehat{A_1}$ to $A_1$ in order to demonstrate some traits of our method and show that we eliminate the need of slice-by-slice study discussed in \cite{kass1990ala}.

The simple roots for the affine $\widehat{A_1}$ are
\begin{equation}
  \label{eq:2}
  (\alpha,\delta-\alpha)
\end{equation}
...
Positive roots of the $\widehat{A_1}$ are $\alpha,\; \alpha+n\delta,\; n\delta,\; -\alpha+n\delta;\; n=1,2,3,\dots$.
The set of the 
...
The set $\Delta_{\bot}$ of the orthogonal positive roots with the zero projection on the root space of the subalgebra $\widehat{A_1}$ is the same as in the finite-dimensional case.

Consider the level one representation of the algebra $\mathfrak{g}=\widehat{B_2}$ with the highest weight $(1,0,1,0)$, where the first to components are the coordinates of the classical part in the orthogonal basis $e_1,e_2$, the third is the grade of the weight and the fourth is the level.
...
\begin{figure}[ph]
  ...
  \caption{The anomalous points of the $(1,0,1,0)$ representation of the algebra $\widehat{B_2}$}
