...
\begin{proof}
To proof the theorem return to the relation (\ref{eq:4}) and rewrite it for the element $\quad
...
\item  Construct the set $\widehat{\Psi ^{(\mu )}}=\left\{ \omega (\mu +\rho
)-\rho ;\;\omega \in W\right\} $ of the anomalous weights for the $\frak{g}$%
-module $L^{(\mu )}$.
