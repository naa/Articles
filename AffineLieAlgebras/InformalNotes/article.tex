...
We prove that for any highest weight $\hf$-diagonalizable module $L^{\left( \mu \right)}$ and orthogonal pair of subalgebras
...
main tools is the set of weights $\Gamma _{\af\subset \frak{g}%
...
\begin{theorem}
  ...
  \begin{equation}
    ...
    \begin{array}{c}
      ...
      \left.
        +\sum_{\gamma \in
          \Gamma _{\af \longrightarrow \gf}}s\left( \gamma +\gamma _{0}\right) k_{\xi
...
\begin{figure}[p]
  ...
  \caption{Regular embedding of $A_1$ into $B_2$. Simple roots $\alpha_1, \alpha_2$ of $B_2$ are presented as the dashed vectors.
    ...
    The superscripts of the highest weights
  $\af_{\perp }^{\mu_{\af_{\perp }}}\left( u\right)$ are the products $\epsilon(w)\dim\left(L_{\af_{\perp }}^{\mu
_{\af_{\perp }}\left( u\right) }\right)$
...
%\begin{figure}[pb]
  ...
  the weights of $\afb=A_1$-modules $L_{\af_{\perp }}^{\mu_{\af_{\perp }}\left( u\right) }$
...
According to formula (\ref{defect-perp}) in our case $\mathcal{D}_{\af_{\perp }}=0$ and
$\hf_{\perp }=0$, thus $\left\{ \mu _{\af_{\perp }}\left( w\right)
=\pi _{\af_{\perp }}\left[ w(\mu +\rho)-\rho \right]\right\}$.
We have four highest weights of $\af_{\perp }$-modules. In terms of
...
2L_{A_1}^{\omega_{\left(A_1\right)1} }
\bigoplus
L_{A_1}^{2\omega_{\left(A_1\right)1} }$.
...
\begin{figure}[h]
  ...
  \caption{The regular embedding $B_2 \longrightarrow B_4$ described by dropping the node from the Dynkin diagram.
...
%\begin{figure}[pb]
  ...
  \caption{The fan $\Gamma$ for $B_2\longrightarrow B_4$ and the values of $s(\gamma+\gamma_0)$ for the weights $\gamma$.}
  \label{fig:B4B2Fan}
\end{figure}

In the orthogonal basis $\left\{e_1,\dots,e_4\right\}$ simple roots and positive roots of $B_4$ are
\begin{equation*}
  \label{eq:8}
...
fan $\Gamma_{B_2 \to B_4}$ can be constructed (using formulas (\ref{eq:6},\ref{eq:37},\ref{fan-defined})).
...
by $\epsilon\left( u \right)$. The results are shown in the Figure \ref{fig:B4B2anom}.
...
\begin{figure}[h!bt]
  ...
  \caption{The fan $\Gamma_{\hat{A_1}\longrightarrow \hat{A_2}}$ for $\hat{A_1}\longrightarrow \hat{A_2}$. The grade is along the horizontal axis and the simple root $\alpha$ of $A_{1}$ is used as the unit of measurement of vertical axis.  Values of  $s(\gamma+\gamma_0)$ are shown for the weights $\gamma\in \Gamma_{\hat{A_1}\longrightarrow \hat{A_2}}$}
...
%  \hspace*{-2cm}
  ...
  \caption{The anomalous weights of the module $L_{\hat{A_2}}^{\omega_0}=L^{(0,0;1;0)}_{\hat{A_2}}$. The standard basis $\left\{e_{1},e_{2}\right\}$ is used for the finite-dimensional subalgebra and the grade is along the horizontal axis. The weights $w (\omega_0+\rho)-\rho$ are marked by crosses when $\epsilon(w)=1$ and
