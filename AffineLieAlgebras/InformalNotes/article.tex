...
There exist several different approaches to deal with the branching coefficients. Some of them use the BGG
...
\begin{theorem}
 ...
 As we have mentioned above (see (\ref{eq:21}) and (\ref
{eq:21-1})) when $\zeta \in \bar{C}_{\frak{a}}$ the coefficients $k_{\zeta
}^{(\mu )}$ coinside with the branching coefficients thus the
equation for $k_{\zeta }^{(\mu )}$ serve as recurrent relations for branching coefficients.
\begin{proof}
To prove the theorem return to the relation (\ref{eq:4}) and rewrite it for the element $\quad
\frac{\Psi _{\frak{g}}^{\mu }}{R_{\frak{a}_{\perp }}}$ ,
\begin{eqnarray*}
\pi _{\frak{a}}\left( \frac{\Psi _{\frak{g}}^{\mu }}{R_{\frak{a}_{\perp }}}%
\right)  &=&\prod_{\alpha \in \Delta ^{+}\setminus \Delta _{\bot
}^{+}}\left( 1-e^{-\pi _{\frak{a}}\alpha }\right) ^{\mathrm{mult}(\alpha )-%
\mathrm{mult}_{\frak{a}}(\pi _{\frak{a}}\alpha )}\left( \sum_{\nu \in P_{%
\frak{a}}^{+}}b_{\nu }^{(\mu )}\sum_{w \in W_{\frak{a}}}\epsilon
(w )e^{w (\nu +\rho _{\frak{a}})-\rho _{\frak{a}}}\right) = \\
&=&-\sum_{\gamma \in \Phi _{\frak{a}\subset \frak{g}}}s(\gamma )e^{-\gamma
}\left( \sum_{\nu \in P_{\frak{a}}^{+},w \in W_{\frak{a}}}\epsilon
(w )b_{\nu }^{(\mu )}e^{w (\nu +\rho _{\frak{a}})-\rho _{\frak{a}%
}}\right).
\end{eqnarray*}
Then decompose the last sum (with respect to the formal basis in $\mathcal{E}$%
):
\begin{equation*}
\pi _{\frak{a}}\left( \frac{\Psi _{\frak{g}}^{\mu }}{R_{\frak{a}_{\perp }}}%
\right) =-\sum_{\gamma \in \Phi _{\frak{a}\subset \frak{g}}}s(\gamma
)e^{-\gamma }\sum_{\lambda \in P_{\frak{a}}}k_{\nu }^{(\mu )}e^{\lambda
}=-\sum_{\gamma \in \Phi _{\frak{a}\subset \frak{g}}}\sum_{\lambda \in P_{%
\frak{a}}}s(\gamma )k_{\nu }^{(\mu )}e^{\lambda -\gamma }.
\end{equation*}
Substitute the expression obtained in the Lemma (in the left-hand side),
\begin{eqnarray*}
\pi _{\frak{a}}\left( \frac{\Psi _{\frak{g}}^{\mu }}{R_{\frak{a}_{\perp }}}%
\right)  &=&\sum_{u\in U}\;\epsilon (u)e^{\pi _{\frak{a}}\left( \mu _{\frak{a%
}}\left( u\right) \right) }\dim \left( L_{\widetilde{\frak{a}_{\perp }}%
}^{\mu _{\widetilde{\frak{a}_{\perp }}}\left( u\right) }\right)
\label{anom modules 2} \\
&=&\sum_{u\in U}\;\epsilon (u)e^{\pi _{\frak{a}}\left[ u(\mu +\rho )-\rho %
\right] }\dim \left( L_{\widetilde{\frak{a}_{\perp }}}^{\mu _{\widetilde{%
\frak{a}_{\perp }}}\left( u\right) }\right)  \\
&=&-\sum_{\gamma \in \Phi _{\frak{a}\subset \frak{g}}}\sum_{\lambda \in P_{%
\frak{a}}}s(\gamma )k_{\nu }^{(\mu )}e^{\lambda -\gamma },
\end{eqnarray*}
Equate the coefficients of exponents in the last equality to get the following result:
\begin{equation}
\sum_{u\in U}\epsilon (u)\dim \left( L_{\widetilde{\frak{a}_{\perp }}}^{\mu
_{\widetilde{\frak{a}_{\perp }}}\left( u\right) }\right) \delta _{\xi ,\pi _{%
\frak{a}}\left[ u(\mu +\rho )-\rho \right] }+\sum_{\gamma \in \Phi _{\frak{a}%
\subset \frak{g}}}s(\gamma )\;k_{\xi +\gamma }^{(\mu )}=0,\quad \xi \in P_{%
\frak{a}}.  \label{eq:17}
\end{equation}
The obtained formula means that the coefficients $k_{\xi +\gamma }^{(\mu )}$
for $\gamma \in \Phi _{\frak{a}\subset \frak{g}}$ are not independent, they
are subject to the linear relations and the form of these relations changes
when the tested weight $\xi $ coinsides with one of \ the ''anomalous
weights'', $\left\{ \pi _{\frak{a}}\left[ u(\mu +\rho )-\rho \right] |u\in
...
injection fan $\Gamma _{\frak{a}\subset \frak{g}}$ (see the definition \ref
...
here the subalgebra $\frak{a}_{\bot }$ may be larger than the one obtained
by elimination of the subdiagram of $\frak{a}$ and the adjacent nodes. The
subalgebras of the orthogonal pair,$\ \frak{a}$ and $\frak{a}_{\bot }$, must
not form a direct sum in $\frak{g}$ . It can be directly
checked that when $\frak{g}=B_{r}$ and $\frak{a}=B_{r_{\frak{a}}}$ the
orthogonal subalgebra is $\frak{a}_{\bot }=B_{r-r_{\frak{a}}}$. Consider the
injection $B_{r_{\frak{a}}}\rightarrow B_{r},\quad 1<r_{\frak{a}}<r$ . By
eliminating the simple root $\alpha _{r_{\frak{a}}-1}=e_{r_{\frak{a}%
}-1}-e_{r_{\frak{a}}}$ one splits the extended Dynkin diagram of $B_{r}$
into the disjoint diagrams for $\frak{a}=B_{r_{\frak{a}}}$ and $D_{r-r_{%
\frak{a}}}$. But the system $\Delta _{\frak{a}_{\perp }}$ contains not only
the simple roots $\left\{ e_{1}-e_{2},e_{2}-e_{3},\ldots ,e_{r_{\frak{a}%
}-2}-e_{r_{\frak{a}}-1},e_{1}+e_{2}\right\} $ but also the root $e_{r_{\frak{%
a}}-1}$. Thus $\Delta _{\frak{a}_{\perp }}$ forms the subsystem of the type $%
B_{r-r_{\frak{a}}}$ and the orthogonal pair for the injection $B_{r_{\frak{a}%
}}\rightarrow B_{r}$ is $\left( B_{r_{\frak{a}}},B_{r-r_{\frak{a}}}\right) $%
...
\begin{enumerate}
\item  Construct the root subsystem $\Delta _{\frak{a}}$ for the embedding $%
\frak{a}\rightarrow \frak{g}$.

\item  Select the positive roots $\alpha \in \Delta ^{+}$ orthogonal to the
roots in $\Delta _{\frak{a}}$ i.e. form the set $\Delta _{\bot }^{+}$.

\item  Construct the set $\Gamma _{\frak{a}\rightarrow \frak{g}}$ (\ref
{fan-defined}).

\item  Construct the set $\widehat{\Psi ^{(\mu )}}=\left\{ w (\mu +\rho
)-\rho ;\;w \in W\right\} $ of the anomalous weights for the $\frak{g}$%
-module $L^{(\mu )}$.

\item  Select the weights $\left\{ \mu _{\widetilde{\frak{a}_{\perp }}%
}\left( w\right) =\pi _{\widetilde{\frak{a}_{\perp }}}\left[ w(\mu +\rho
)-\rho \right] -\mathcal{D}_{\frak{a}_{\perp }}\in \overline{C_{\widetilde{%
\frak{a}_{\perp }}}}\right\} $. Since the set $\Delta _{\bot }^{+}$ is fixed
we can easily check wether the weight $\mu _{\widetilde{\frak{a}_{\perp }}%
}\left( w\right) $ belongs to the main Weyl chamber $\overline{C_{\widetilde{%
\frak{a}_{\perp }}}}$ (by computing its scalar product with the roots from $%
\Delta _{\bot }^{+}$).

\item  For the weights $\mu _{\widetilde{\frak{a}_{\perp }}}\left( w\right) $
calculate the dimensions of the corresponding modules $\mathrm{\dim }\left(
L_{\widetilde{\frak{a}_{\perp }}}^{\mu _{\widetilde{\frak{a}_{\perp }}%
}\left( u\right) }\right) $.

\item  Calculate the anomalous branching coefficients in the main Weyl
chamber $\overline{C_{\frak{a}}}$ of the subalgebra $\frak{a}$ using the
...
%%  
%%  \begin{definition}
%%    Denote by $W_{\bot}$ the subgroup of the Weyl group $W$ generated by the reflections $w_{\alpha}$ corresponding to the roots $\alpha\in \Delta^{+}_{\bot}$.
%%  \end{definition}
%%  \begin{mynote}
...
%%    \begin{equation*}
%%      \begin{array}{cc}
%%        vu(\mu+\rho)-\rho & = v\cdot(\piab(u(\mu+\rho))-\rho_{\afb}+\rho_{\afb})-\rho_{\afb}+\\
%%        & \pia(u(\mu+\rho))-\rho+\rho_{\afb}+
%%        \pi_{\mathfrak{h}_d^*}(u(\mu+\rho))
%%      \end{array}
%%    \end{equation*}
...
%%      \sum_{u\in W/W_{\bot}}\epsilon(u)e^{\pia(u(\mu+\rho))-\rho+\rho_{\afb}+\pi_{\mathfrak{h}_d^*}(u(\mu+\rho))}\cdot\\
%%  
%%      &\cdot\frac{
...
%%      \begin{equation}
%%        \label{eq:4}
%%        \pi_{\mathfrak{a}}\left(\frac{\sum_{w\in W} \epsilon(w) e^{w(\mu+\rho)-\rho}}
%%          {\prod_{\alpha\in\Delta^{+}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)}}\right) =
%%        \sum_{\nu\in P^{+}_{\mathfrak{a}}}b^{(\mu)}_{\nu}
%%        \frac{\sum_{w\in W_{\mathfrak{a}}}\epsilon(w)
...
%%        \frac{\sum_{w\in W_{\bot}} \epsilon(w)
%%          e^{w(\mu+\rho_{\mathfrak{a}_{\bot}})-\rho_{\mathfrak{a}_{\bot}}}}
...
%%      Consider the factor-space $W_{\bot}\backslash W$. For the class $\tilde{w}\in
%%      W_{\bot}\backslash W$ choose the representative $w \in \tilde{w}$
%%      such that $\pi_{\mathfrak{a}_{\bot}}w(\mu+\rho)\in \bar{C}_{\mathfrak{a}_{\bot}}$,
%%      \begin{equation}
%%        \label{eq:13}
%%        \begin{array}{c}
%%          \fl \pi_{\mathfrak{a}}\left(\frac{\sum_{w\in W} \epsilon(w) e^{w(\mu+\rho)-\rho}}
%%            {\prod_{\alpha\in\Delta^{+}_{\bot}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)}}\right) = \\
%%          = \pi_{\mathfrak{a}}\left(\sum_{w\in W_{\bot}\backslash W} \epsilon(w)
%%            \frac{\sum_{\nu\in W_{\bot}}\epsilon(\nu) e^{\nu \cdot w(\mu+\rho)-\rho}}
%%            {\prod_{\alpha\in\Delta^{+}_{\bot}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)}}\right).
%%        \end{array}
%%      \end{equation}
%%      The fraction in the right-hand side of the equation looks like the character of some  $\mathfrak{a}_{\bot}$-module.
%%      Since $\nu\cdot \pi_{\mathfrak{a}}(w(\mu+\rho))=\pi_{\mathfrak{a}}(w(\mu+\rho))$ and
%%      $w(\mu+\rho)-\pi_{\mathfrak{a}}(w(\mu+\rho))=\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))$,
%%      we can rewrite the shifted weights
%%      \begin{equation*}
%%        \label{eq:30}
%%        \nu\cdotw(\mu+\rho)-\rho=\nu\cdot \bigl(\pi_{\mathfrak{a}_{\bot}}
%%        (w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}+\rho_{\mathfrak{a}_{\bot}}
%%        +\pi_{\mathfrak{a}}(w(\mu+\rho))\bigr)-\rho,
%%      \end{equation*}
%%  
%%      \begin{eqnarray*}
%%        \label{eq:14}
%%        \fl  \sum_{w\in W_{\bot}\backslash W} \epsilon(w) \frac{\sum_{\nu\in W_{\bot}}\epsilon(\nu)
%%          e^{\nu \cdot w(\mu+\rho)-\rho}}{\prod_{\alpha\in\Delta^{+}_{\bot}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)}}
%%        =\\
%%        \sum_{w\in W_{\bot}\backslash W} \epsilon(w) e^{\pi_{\mathfrak{a}}(w(\mu+\rho))-\rho}
%%        \frac{e^{\rho_{\mathfrak{a}_{\bot}} }\sum_{\nu\in W_{\bot}}\epsilon(\nu) e^{\nu \cdot
%%            (\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}+\rho_{\mathfrak{a}_{\bot}})
%%            -\rho_{\mathfrak{a}_{\bot}}}}{\prod_{\alpha\in\Delta^{+}_{\bot}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)}}
%%        =\\
%%        \sum_{w\in W_{\bot}\backslash W} \epsilon(w) e^{\pi_{\mathfrak{a}}(w(\mu+\rho))-\rho}
%%        e^{\rho_{\mathfrak{a}_{\bot}}} {\rm ch} L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))
%%          -\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}.
%%      \end{eqnarray*}
%%      The projector $\pi_{\mathfrak{a}}$ sends the character
%%      ${\rm ch}L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}$
%%      to the unit element of $\cal{E}$ multiplied by
%%      ${\rm dim}L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}$ :
%%      \begin{eqnarray*}
%%        \label{eq:15}
%%        \fl    \pi_{\mathfrak{a}}\left( \sum_{w\in W_{\bot}\backslash W} \epsilon(w)
%%          e^{\pi_{\mathfrak{a}}(w(\mu+\rho))-\rho}e^{\rho_{\mathfrak{a}_{\bot}}} \mathrm{ch}
%%          L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right)
%%        = \\
%%        \sum_{w\in W_{\bot}\backslash W} \epsilon(w)\;
%%        \mathrm{dim}\left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))
%%            -\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right) e^{\pi_{\mathfrak{a}}(w(\mu+\rho)-\rho)}.
%%      \end{eqnarray*}
%%      Thus we obtain the relation
%%      \begin{equation}
%%        \label{eq:9}
%%        \begin{array}{c}
%%          \fl  \sum_{w\in W_{\bot}\backslash W} \epsilon(w) \mathrm{dim}\left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right) e^{\pi_{\mathfrak{a}}(w(\mu+\rho)-\rho)}=\\
...
%%      \begin{eqnarray*}
%%        \label{eq:16}
%%        \sum_{w\in W_{\bot}\backslash W} \epsilon(w) \mathrm{dim}\left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right) e^{\pi_{\mathfrak{a}}(w(\mu+\rho)-\rho)}=\\
...
%%        \label{eq:17}
%%        \begin{array}{c}
%%          \fl \sum_{w\in W_{\bot}\backslash W} \epsilon(w) \mathrm{dim}
%%          \left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))
%%              -\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right)
%%          \delta_{\xi,\pi_{\mathfrak{a}}(w(\mu+\rho)-\rho)}+
...
%%        \left(
%%          \sum_{w\in W_{\bot}\backslash W} \epsilon(w)\; \mathrm{dim}
%%          \left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))-\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right)
%%          \delta_{\xi-\gamma_0,\pi_{\mathfrak{a}}(w(\mu+\rho)-\rho)}+
%%        \right.\\
%%        \left. +
...
%%  are reduced to the element $e^{ \pi_{\mathfrak{a}_{\bot}}w(\mu+\rho)}$.
%%  In the first two cases the vector $w(\mu+\rho)$ is projected to the subspace orthogonal to the weight space of $\mathfrak{a}$. It is clear that in any of the three variants the final vector $\pi_{\mathfrak{a}}\pi_{\mathfrak{a}_{\bot}}w(\mu+\rho)$ leads to the unit of the formal algebra
...
%%  \item Construct the set $\widehat{\Psi^{(\mu)}}=\left\{w(\mu+\rho)-\rho;\; w\in W\right\}$ of the anomalous weights for the $\mathfrak{g}$-module $L^{(\mu)}$.
%%  \item Select the weights $\left\{ \lambda=w(\mu+\rho) | \pi_{\mathfrak{a}_{\bot}}\lambda \in \bar{C}_{\mathfrak{a}_{\bot}} \right\}$.
...
%%  \item For $\lambda=w(\mu+\rho),\; \pi_{\mathfrak{a}_{\bot}}\lambda\in \bar{C}_{\mathfrak{a}_{\bot}}$ calculate
%%  the dimensions of the corresponding modules $\mathrm{dim}\left(L^{\pi_{\mathfrak{a}_{\bot}}(w(\mu+\rho))
%%  -\rho_{\mathfrak{a}_{\bot}}}_{\mathfrak{a}_{\bot}}\right)$.
%%  \item Calculate the anomalous branching coefficients in the main Weyl
