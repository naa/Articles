...
We show that our algorithm can be used in a study of conformal embeddings where the central charge of the conformal field theory is preserved, and the computations are simplified taking into account some  physical limitations.
...
Our aim is to demonstrate that despite possible singularities in the Weyl denominator projected to the subalgebra root space injection fan technique can be properly modified. as a result of such modification the generalized recurrent relations for anomalous branching coefficients \eqref{eq:21} will appear in the following form: 
\begin{equation}
  \label{eq:1}
  \begin{array}{c}
...
+\gamma }^{\left( \mu \right) }\right)
  \end{array}
   \end{equation}
Here $\mathfrak{a}_{\bot}$ is the subalgebra generated by the roots of $\mathfrak{g}$ orthogonal to the root system of $\mathfrak{a}$, $W_{\bot}$ is the corresponding Weyl group, $\Gamma_{\mathfrak{a}\subset \mathfrak{g}}$ is the set of weights in the expansion of the denominator $\prod_{\alpha\in \Delta^{+}\setminus \Delta^{+}_{\mathfrak{a}_{\bot}}}(1-e^{-\alpha})^{\mathrm{mult}(\alpha)-\mathrm{mult}_{\mathfrak{a}}(\alpha)}$ and $s(\gamma)$ is the coefficient of $e^{\gamma}$ in this expansion.
In the next subsection we study the situation in details and prove the validity of this relation.

In the next section we  shall describe the  computational algorithm for  branching coefficients based on this formula and present some examples.

\subsection{Proof of the recurrent relation}
\label{sec:proof}

The branching of a module can be symbolically written using formal characters and projection operators $\pi_{\mathfrak{a}}$ that bring the roots of $\mathfrak{g}$ to the weight subspace of $\mathfrak{a}$:
\begin{equation}
  \label{eq:3}
  L_{\frak{g}\downarrow \frak{a}}^{\mu }=\bigoplus\limits_{\nu \in P_{\frak{a}%
    }^{+}}b_{\nu }^{\left( \mu \right) }L_{\frak{a}}^{\nu }\quad
...
It is important to keep in mind that the projection of some of positive roots of the algebra $\mathfrak{g}$ can be equal to zero. These roots are orthogonal to the root space of the subalgebra $\mathfrak{a}$ embedded into the root space of the algebra $\mathfrak{g}$. Let's denote the subset of such roots by $\Delta^{+}_{\bot}=\left\{\alpha\in\Delta_{\mathfrak{g}}^{+}:\forall \beta\in \Delta_{\mathfrak{a}}^{+},\; \alpha\bot\beta \right\}$.

Notice that if the set $\Delta^{+}_{\bot}$ is non-empty the Weyl reflections corresponding to the positive roots of $\Delta^{+}_{\bot}$ generate a subgroup $W_{\bot}$ of the Weyl group $W$. Consider any two positive roots $\alpha,\; \beta\in \Delta^{+}_{\bot}$ and the corresponding Weyl reflections $\omega_{\alpha},\; \omega_{\beta}\in W_{\bot}$.  Since roots of the subalgebra $\mathfrak{a}$ are invariant under $\omega_{\alpha}, \; \omega_{\beta}$ they are also invariant under the action of $\omega_{\gamma}=\omega_{\alpha}\cdot \omega_{\beta}$. So the subgroup $W_{\bot}$ preserves the root system of the subalgebra $\mathfrak{a}$. 

Thus we have obtained the root system $\Delta_{\bot}$ which is orthogonal to the root system $\Delta_{\mathfrak{a}}$ and invariant with respect to  $W_{\bot}$ . This root system can be considered as the root system of a subalgebra $\mathfrak{a}_{\bot}\subset \mathfrak{g}$.

Now we should find out when the subset $\Delta^{+}_{\bot}$ is non-empty and the subgroup $W_{\bot}$ and subalgebra $\mathfrak{a}_{\bot}$ are non-trivial.

If $\mathfrak{a}$ is a maximal regular subalgebra of $\mathfrak{g}$ then the rank of $\mathfrak{a}$ is equal to the rank of $\mathfrak{g}$ and it is clear that $\Delta^{+}_{\bot}$ is empty.
On the other hand 
non-maximal regular embedding of $\mathfrak{a}$ into $\mathfrak{g}$ can be obtained through the chain of maximal embeddings $\mathfrak{a}\subset \mathfrak{p}_1\subset \mathfrak{p}_2\subset\dots \subset \mathfrak{g}$. The maximal regular embeddings are constructed by the exclusion of one or two roots from the extended Dynkin diagram of the algebra. Since this process can give us non-connected Dynkin diagrams we can see which roots are orthogonal to the root space of non-maximal regular subalgebra $\mathfrak{a}$. 

Consider for instance regular the embedding of $A_1\subset B_2$.

The extended Dynkin diagram of $B_2$ is presented in the Figure 1.
\begin{figure}[ph]
  \centering
  ...
  \label{fig:B2Dynkin}
\end{figure}
When we drop the central node and get the embedding $A_1\oplus A_1\subset B_2$. Then $\mathfrak{a}=A_1$ and $\mathfrak{a}_{\bot}=A_1$.

Here we present the simple criterion of $\Delta^{+}_{\bot}$'s non-emptiness for a regular embedding $\mathfrak{a}\subset \mathfrak{g}$ when both $\mathfrak{a}$ and $\mathfrak{g}$ are simple. If the Dynkin diagram of $\mathfrak{g}$ can be split into the disconnected diagrams of $\mathfrak{a}$ and some other subalgebras $\{\mathfrak{\bar{a}}_j\}$ then the subset $\Delta_{\bot}$ is non-empty, subalgebra $\mathfrak{a}_{\bot}$ is non-trivial and all the $\mathfrak{\bar{a}}_j$ are the subalgebras of $\mathfrak{a}_{\bot}$.

Note that when we study the regular embedding obtained by dropping the nodes of the extended Dynkin diagram of the algebra $\mathfrak{g}$ and the subalgebra $\mathfrak{a}$ is one of the connected components, the subalgebra $\mathfrak{a}_{\bot}$ may be broader than the algebra generated by the other connected components. See for example the figure \ref{fig:dynkin} for the embedding of $B_2\subset B_4$. 
In this case the extended Dynkin diagram of $B_4$ is split by dropping the simple root $\alpha_2=e_2-e_3$ into the diagrams of the subalgebra of interest $\mathfrak{a}=B_2$ and two diagrams of the orthogonal $A_1$ subalgebras. But the subalgebra $\mathfrak{a}_{\bot}$ is equal not to $A_1\oplus A_1$ but to $B_2$. 
This fact is due to the existence of the positive root $e_2=s_{\alpha_2}\cdot s_{\alpha_3}\cdot\alpha_4$ in the root system of $B_4$.
...
The exceptional Lie algebras require tedious case-by-case analysis.

In terms of Weyl groups the task of construction of the root system of $\mathfrak{a}_{\bot}$ is formulated as follows. Given a Weyl group $W$ and a subgroup $W_{\mathfrak{a}}$ find the subgroup $W_{\mathfrak{a}_{\bot}}\subset W$ such that $\omega\, W_{\mathfrak{a}}\,\omega^{-1}=W_{\mathfrak{a}}; \;\text{for}\; \omega\in W_{\mathfrak{a}_{\bot}}$, so we need to find the normalizer of $W_{\mathfrak{a}}$ in $W$, such that some of the elements of $W_{\mathfrak{a}_{\bot}}$ are the reflections corresponding to the positive roots of $\mathfrak{g}$. This task can be carried out using the computational methods of finite group theory \cite{stembridge2001computational} but we are unaware of any publication with the exhaustive tables of such subgroups.
...
\begin{equation}
  ...
  \end{array}
\end{equation}
Exceptional Lie algebras and other non-maximal subalgebras require lengthy analysis which is not important for the needs of computation of the branching coefficients.
...
It is easy to see that the set $\Delta_{\bot}$ is trivial. 
\subsubsection{Special embedding $\hat{su}(2)\subset \hat{sp}(4)$.}
\label{sec:spec-embedd-wideh}
