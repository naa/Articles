...
\af}}s(\gamma )k_{\nu }^{(\mu )}e^{\lambda -\gamma }.
\end{equation*}
Substitute in the left-hand side the expression obtained in Lemma \ref{lemma},
\begin{eqnarray*}
\Psi ^{\left( \mu \right) }_{\left(  \af, \afb \right)}
...
fan $\Gamma_{B_2 \to B_4}$ can be constructed using Definition \ref{fan-defininition}.
