...
(when it is projected to $P_{\mathfrak{a}}$) the injection fan technique \cite{ilyin812pbc} can be properly modified.
...
orthogonal to $P_{\mathfrak{a}}$, $W_{\bot}$ is the corresponding Weyl group,
...
Consider the positive roots $\alpha\in \Delta$ orthogonal to $P_{\mathfrak{a}}$.
Denote the subset of such roots by $\Delta^{+}_{\bot}
...
are invariant with respect to $\omega_{\alpha}, \; \omega_{\beta}$
and $\omega_{\gamma}=\omega_{\alpha}\cdot \omega_{\beta}$
the subgroup $W_{\bot}$ preserves the roots in $\Delta_{\mathfrak{a}}$.
The subsystem $\Delta_{\bot}$
can be considered as the root system of a subalgebra $\mathfrak{a}_{\bot}\subset \mathfrak{g}$.

Now we are to find out when the subset $\Delta^{+}_{\bot}$ is nonempty
with the nontrivial subgroup $W_{\bot}$ and the subalgebra $\mathfrak{a}_{\bot}$.

If $\mathfrak{a}$ is a maximal regular subalgebra of $\mathfrak{g}$ then $r_{\mathfrak{a}}=r$ and
 $\Delta^{+}_{\bot}$ is empty.
On the other hand
nonmaximal regular embedding of $\mathfrak{a}$ into $\mathfrak{g}$ can be obtained
...
nonconnected diagrams we immediately fix the roots orthogonal to $P_{\mathfrak{a}}$.
...
For a regular embedding $\mathfrak{a}\subset \mathfrak{g}$ when both $\mathfrak{a}$
and $\mathfrak{g}$ are simple the sufficient condition for $\Delta^{+}_{\bot}$ to be
nontrivial can be formulated as follows:
...
then the subset $\Delta_{\bot}$ is not empty,
subalgebra $\mathfrak{a}_{\bot}$ is nontrivial and
...
\end{table}
If $\mathfrak{a}=B_{r_{\mathfrak{a}}}$  we see that $\Delta_{\mathfrak{a}_{\bot}}=\{\alpha_{\pm i,\pm j},\;\alpha_j,\;
1<i<j\leq r-r_{\mathfrak{a}}\}$ and $\mathfrak{a}_{\bot}=B_{r-r_{\mathfrak{a}}}$.
This is the only case where the simple roots of $\mathfrak{a}_{\bot}$ can not be
obtained from the extended Dynkin diagram, as can be seen in the Table \ref{tab:diagrams}.

Using the existing classification of maximal special subalgebras in classical Lie algebras
\cite{dynkin1952semisimple} we have the following pairs of the orthogonal
subalgebras $\mathfrak{a},\;\mathfrak{a}_{\bot}$
\begin{equation*}
%  \label{eq:42}
  \begin{array}{lll}
...
Now consider the direct sum $\mathfrak{a}_{\bot}\oplus\mathfrak{h}_{\mathfrak{a}}$ and
...
\end{equation}
and for the coefficient function $s\left( \gamma \right) $ define the carrier $\Phi _{\frak{a%
}\subset \frak{g}}\subset P_{\frak{a}}$:
\begin{equation}
\Phi _{\frak{a}\subset \frak{g}}=\left\{ \gamma \in P_{\frak{a}}\mid s\left(
\gamma \right) \neq 0\right\} .  \label{phi-d}
\end{equation}
From the relation (\ref{eq:9}) the corresponding equation in terms of formal elements,
\begin{eqnarray*}
  \label{eq:16}
...
\end{eqnarray*}
leads to the following property of the anomalous branching coefficients,
\begin{equation}
\label{eq:17}
\begin{array}{c}
