...
_{_{\af}})-\rho _{_{\af}}}$ --- the corresponding singular weight
element for the $\af$-module $L_{\af}^{\nu }$;
...
\noindent $\left( \mbox{resp. }R_{\af}:=\prod_{\alpha \in \Delta _{%
\af}^{+}}\left( 1-e^{-\alpha }\right) ^{\mathrm{mult}_{\af}\mathrm{%
\left( \alpha \right) }}=\Psi _{\af}^{\left( 0\right) }\right) $--- the
denominator;

$ L_{\frak{g}\downarrow \af}^{\mu }=\bigoplus
\limits_{\nu \in P_{\af}^{+}}b_{\nu }^{\left( \mu \right) }L_{\af}^{\nu }$
 --- the module decomposition with respect to $\af\longrightarrow \frak{g}$;
...
Let $\af\subset \frak{g}$ be a regular subalgebra of $\frak{g.}$
Applying the Weyl-Kac character formula to the branching rule $\pi _{\af%
}ch\left( L^{\mu }\right) =\sum_{\nu \in P_{\af}^{+}}b_{\nu }^{(\mu
)}ch\left( L_{\af}^{\nu }\right) $ we get the relation connecting the
singular elements $\Psi ^{\left( \mu \right) }$ and $\Psi _{\left( \af%
\right) }^{\left( \nu \right) }$ :
\begin{eqnarray}
...
a}\right) }^{\left( \nu \right) }}{R_{\af}}.
\end{eqnarray}
where according to our notations $\Delta _{\af}^{+}$ is the set of
positive roots of the subalgebra $\af$ (it is convenient to choose them
from the positive root subspace of $\frak{g}$). Consider the root space $%
\frak{h}_{\perp \af}^{\ast }$ orthogonal to $\Delta _{\af}$,
\begin{equation*}
\frak{h}_{\perp \af}^{\ast }=:\left\{ \eta \in \frak{h}_{\frak{g}%
}^{\ast }|\forall \alpha \in \Delta _{\af};\alpha \bot \eta \right\} ,
\end{equation*}
...
The algebras $\af$ and $\af_{\perp }$ form an ''orthogonal pair''
...
\frak{h}_{\af}.
\end{equation}
For the subalgebras of the orthogonal pair $\left( \af,\af_{\perp
...
\right) }}$ and their projections to $h_{\widetilde{\af_{\perp }}%
}^{\ast }$ (additionally shifted by $-\mathcal{D}_{\af_{\perp }}$):
\begin{equation*}
\mu _{\widetilde{\af_{\perp }}}\left( w\right) :=\pi _{\widetilde{\frak{%
a}_{\perp }}}\left[ w(\mu +\rho )-\rho \right] -\mathcal{D}_{\af_{\perp
}},\quad w\in W.
\end{equation*}
Among the weights $\mu _{\widetilde{\af_{\perp }}}\left( w\right)
...
\begin{equation*}
\mu _{\af}\left( u\right) :=\pi _{\af}\left[ u(\mu +\rho )-\rho %
...
)\right) -\rho +\rho _{\af_{\perp }}+v\left( \pi _{\left( \widetilde{%
\af_{\perp }}\right) }u(\mu +\rho )-\rho _{\af_{\perp }}+\rho _{%
\af_{\perp }}\right) -\rho _{\af_{\perp }}
\end{equation*}
...
\begin{equation*}
\begin{array}{l}
\Psi _{\frak{g}}^{\mu }=\sum_{v\in W_{\af_{\perp }}}\sum_{u\in
U}\epsilon (v)\epsilon (u)e^{vu(\mu +\rho )-\rho }= \\
=\sum_{u\in U}\epsilon (u)e^{\pi _{\af}\left( u(\mu +\rho )\right)
-\rho +\rho _{\af_{\perp }}}\sum_{v\in W_{\af_{\perp }}}\epsilon
(v)e^{v\left( \pi _{\left( \widetilde{\af_{\perp }}\right) }u(\mu +\rho
)-\rho _{\af_{\perp }}+\rho _{\af_{\perp }}\right) -\rho _{\af%
_{\perp }}}= \\
=\sum_{u\in U}\epsilon (u)e^{\pi _{\af}\left( u(\mu +\rho )\right) -\pi
_{\frak{h}_{\perp }}\rho -\pi _{\af}\rho +\mathcal{D}_{\af_{\perp
}}}\sum_{v\in W_{\af_{\perp }}}\epsilon (v)e^{v\left( \pi _{\left(
\widetilde{\af_{\perp }}\right) }u(\mu +\rho )-\rho _{\af_{\perp
}}+\rho _{\af_{\perp }}\right) -\rho _{\af_{\perp }}}= \\
=\sum_{u\in U}\epsilon (u)e^{\pi _{\af}\left[ u(\mu +\rho )-\rho \right]
+\mathcal{D}_{\af_{\perp }}}\sum_{v\in W_{\af_{\perp }}}\epsilon
(v)e^{v\left( \pi _{\left( \widetilde{\af_{\perp }}\right) }u(\mu +\rho
)-\rho _{\af_{\perp }}+\rho _{\af_{\perp }}\right) -\pi _{\frak{h}%
_{\perp }}\rho -\rho _{\af_{\perp }}}= \\
=\sum_{u\in U}\epsilon (u)e^{\pi _{\af}\left[ u(\mu +\rho )-\rho \right]
+\mathcal{D}_{\af_{\perp }}}\sum_{v\in W_{\af_{\perp }}}\epsilon
...
{eq:21-1})) when $\zeta \in \bar{C}_{\af}$ the coefficients $k_{\zeta
...
\frac{\Psi _{\frak{g}}^{\mu }}{R_{\af_{\perp }}}$ ,
\begin{eqnarray*}
\pi _{\af}\left( \frac{\Psi _{\frak{g}}^{\mu }}{R_{\af_{\perp }}}%
...
\begin{enumerate}
\item  Construct the root subsystem $\Delta _{\af}$ for the embedding $%
