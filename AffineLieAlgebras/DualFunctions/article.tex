...
Consider the (affine) Lie algebra $\mathfrak{g}$  and its reductive subalgebra $\mathfrak{a}\subset \mathfrak{g}$. Introduce the following objects.
\begin{definition}
...
\begin{lemma}
The quotient of the singular weights element $\Psi^{(\mu)}$ by the denominator $R_{\mathfrak{a}_{\bot}}$ is given by the formal element corresponding to the modified set of the singular weights of $L_{\mathfrak{a}\oplus\mathfrak{h}_d}^{(\pi_{\mathfrak{a}\oplus\mathfrak{h}_d}\mu)}$ with the multiplicities given by the dimensions of $\mathfrak{a}_{\bot}$-modules.
\end{lemma}
