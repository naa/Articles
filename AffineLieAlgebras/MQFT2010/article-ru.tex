...
\author{В Д Ляховский$^1$, А А Назаров$^{2,3}$\\
  {\small $^1$ Теоретический отдел,}\\
  {\small Санкт - Петербургский государственный университет}\\
  {\small 198904, Санкт - Петербург, Россия}\\
  {\small e-mail: lyakh1507@nm.ru}\\
  {\small $^2$ Теоретический отдел,}\\
  {\small Санкт - Петербургский государственный университет}\\
  {\small 198904, Санкт - Петербург, Россия}\\
  {\small $^2$ Лаборатория имени Чебышева,}\\
  {\small математико-механический факультет,}\\ 
  {\small Санкт - Петербургский государственный университет}\\
  {\small 199178, Санкт - Петербург, Россия}\\
...
\begin{statement}
%\bigskip
Для ортогональной подалгебры  $\frak{a}_{\perp }$ в $\frak{g}$ (являющейся ортогональным партнером редуктивной подалгебры $\frak{a}\hookrightarrow \frak{g}$) характера интегрируемого модуля старшего веса  $L^{\mu }$ может быть представлен в виде комбинации (с целочисленными коэффициентами) параболических модулей Верма, распределенных по множеству весов $e^{\mu _{\widetilde{\mathfrak{a}}}\left(
u\right) }$:
\begin{equation}
...
\Delta _{I}^{+}\right\} $, и соответствующие нильпотентные подалгбры в  $\frak{n}%
^{+}$:
\begin{equation*}
...
\end{equation*}
Полупрямое произведение  $\frak{l}_{I}$ и $\frak{u}_{I}^{+}$ дает параболическую подалгебры $\frak{p}_{I}\hookrightarrow \frak{g}$ :
\begin{equation}
...
\end{equation*}
Значит мы получили обобщенную формулу Вейля-Верма для характеров -- разложение  $\mathrm{ch}\left(
...
\begin{remark}
Здесь обобщенная формула Вейля-Верма для характеров (называемая формулой переменного суммирования в книге \cite{humphreys2008representations}) имеет специальный вид: веса $\mu _{\aft}$ и старшие веса обобщенных модулей Верма $\mu _{\afb}$ разделены. Причина в том, что старший вес $M_{I}$-модуля не равен проекции его максимального веса на  $h^*_{\afb}$ (а должен быть дополнительно сдвинут на дефект).
\end{remark}
...
\begin{example}
  ...
  \caption{Обобщенные модули Верма для регулярного вложения  $A_1$ в $B_2$. 
    Простые корни $\alpha_1, \alpha_2$ алгебры $B_2$ показаны пунктирными стрелками. Простой корень  $\beta = \alpha_1+2\alpha_2$ алгебры $A_1$ показан серым вектором. Разложение $L^{\omega_{1}}$ представлено набором контуров входящих в него обобщенных модулей Верма. Пунктирные контуры соответствуют положительным значениям  $\epsilon(u)$, а точечные -- отрицательным. }
...
В работе \cite{lepowsky1977generalization} показано, что для модуля старшего веса $L^{\mu }$, где $\mu \in P^{+}$, последовательность
\begin{equation}
...
\end{equation}
(обобщенная БГГ резольвента), является точной и формула (\ref{gen Weyl-Verma}%
) следует из этого разложения.

\begin{figure}[h!bt]
 \noindent\centering{
   \includegraphics[width=140mm]{B2_Exact.pdf}}
 \caption{Вложение $A_1\hookrightarrow B_2$ (см. Рисунок \ref{fig:B2_Verma_Decomp}). Ортогональный партнер -- это подалгебра $A_1$, соответствующая корню $\alpha_1$.
...
\begin{statement}
Пусть $L^{\mu }$ --  $\frak{g}$-модуль со старшим весом $\mu \in P^{+}$, и пусть регулярная подалгебра $\mu \in P^{+}$ ортогональна редуктивной подалгебре $\frak{a}\hookrightarrow \frak{g}$. Тогда разложение (\ref{sing decomp main}) определяет как обобщенную резольвенту $L^{\mu }$ по отношению к $\frak{a}_{\perp }$, так и правила ветвления $L^{\mu }$ по отношению к $\frak{a}$ .
\end{statement}
...
}}\left( u\right) $ и $\mathcal{D}_{\frak{a}_{\perp }}$ определены как в Лемме \ref{Psi-decomp-lemma}, а  $%
u\in U$ определено формулой (\ref{U-def}). Тогда мы получаем элементы фильтрующей последовательности (\ref{resolution sequence}).
...
u\left( \xi \right) \right) }$ теперь определено:
\begin{equation*}
...
В работе \cite{2010arXiv1007.0318L} было показано, что метод веера вложения работает также и для специальных вложений. Надо заметить, что в этом случае разложения Вейля-Верма тоже могут быть получены. Резольвенты, соответствующие специальным подалгебрам, описывают соотношения между проекциями характера начального модуля и обобщенными модулями Верма со старшими весами в подпространстве $h^*$.
...
Авторы выражают свою искреннюю благодарность всем, кто подготовил и организовал III международную конференцию ``Модели квантовой теории поля 2010'', посвященную 70-летию А.Н. Васильева. 

Данная работа была частично поддержана грантом РФФИ № 09-01-00504 и лабораторией имени Чебышева (математико-механический факультет СПбГУ) из средств гранта 11.G34.31.2006 правительства Российской Федерации.
