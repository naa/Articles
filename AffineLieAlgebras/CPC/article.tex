...
Consider Lie algebra $\gf$ of finite or affine type. We denote its root system by $\Delta$. Roots and weights of the algebra are represented by the data-structures \lstinline{finiteWeight} and \lstinline{affineWeight} of {\it Affine.m} package. 
Root system is determined by the set of simple roots $\alpha_{1}, \dots, \alpha_{r}$, where $r$ is the rank of the algebra. So to specify an algebra it is enough to have simple roots. The set of simple roots can be created using different methods. The coordinates of simple roots can be entered by hand, for example direct sum $sl_{2}\oplus sl_{2}$ can be constructed by the code
\begin{lstlisting}
...
\begin{lstlisting}
  ...
  Out[6]= finiteRootSystem[2, {finiteWeight[2, {1, -1}], finiteWeight[2, {0, 1}]}]
\end{lstlisting}
